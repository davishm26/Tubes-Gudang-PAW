                    ...
                    @else
                        <div class="space-y-4">
                            @foreach($requests as $request)
                                <div class="border border-[#E5E7EB] rounded-lg {{ !$request['is_read'] ? 'bg-[#E9F6F1]' : '' }}">
                                    <!-- Header -->
                                    <div class="px-6 py-4 border-b border-[#E5E7EB] bg-[#E9F6F1]/50">
                                        <div class="flex items-start justify-between">
                                            <div class="flex items-start gap-3">
                                                ...
                                                @endif
                                                <div>
                                                    <h4 class="text-lg font-semibold text-gray-900">{{ $request['company_name'] }}</h4>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                                ...
                                                <div class="space-y-2 text-sm">
                                                    ...
                                                    @if($request['phone'])
                                                        <div class="flex items-center gap-2">
                                                            ...
                                                            <a href="tel:{{ $request['phone'] }}" class="text-blue-600 hover:underline">{{ $request['phone'] }}</a>
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
                                            ...
                                            @if($request['company'])
                                                <div class="bg-white rounded-lg border border-gray-200 p-4">
                                                    ...
                                                    <div class="space-y-2 text-sm">
                                                        @if($request['company']->suspend_reason)
                                                            <div>
                                                                <span class="font-medium text-gray-700">Alasan Suspend:</span>
                                                                <p class="text-gray-600 mt-1">{{ $request['company']->suspend_reason }}</p>
                                                            </div>
                                                        @endif
                                                        @if($request['company']->subscription_price)
                                                            <div class="flex justify-between">
                                                                <span class="font-medium text-gray-700">Harga Langganan:</span>
                                                                <span class="text-gray-900">Rp {{ number_format($request['company']->subscription_price, 0, ',', '.') }}</span>
                                                            </div>
                                                        @endif
                                                    </div>
                                                </div>
                                    ...
                                    @if($isSuspended && $isUnread)
                                        <div class="px-6 py-4 bg-[#E9F6F1] border-t border-[#E5E7EB] flex items-center justify-end gap-3">
                                            ...
                                            <!-- Approve Button -->
                                            <form action="{{ route('super_admin.reactivation.approve', $request['company']->id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit"
                                                    onclick="return confirm('Apakah Anda yakin ingin mengaktifkan kembali akun perusahaan {{ $request['company_name'] }}?')"
