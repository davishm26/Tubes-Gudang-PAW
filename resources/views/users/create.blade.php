<x-app-layout>
    <x-slot name="title">Tambah Pengguna - StockMaster</x-slot>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-900 leading-tight">{{ __('Tambah Pengguna') }}</h2>
    </x-slot>
                ...
                <form method="POST" action="{{ route('users.store') }}">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" class="mt-1 block w-full border border-emerald-200 rounded-xl px-4 py-2 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" required>
                            @error('name')<p class="text-sm text-rose-600 mt-1">{{ $message }}</p>@enderror
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-700">Email</label>
                            <input type="email" name="email" value="{{ old('email') }}" class="mt-1 block w-full border border-emerald-200 rounded-xl px-4 py-2 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" required>
                            @error('email')<p class="text-sm text-rose-600 mt-1">{{ $message }}</p>@enderror
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-700">Password</label>
                            <input type="password" name="password" class="mt-1 block w-full border border-emerald-200 rounded-xl px-4 py-2 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" required>
                        ...
                        <div>
                            ...
                            <input type="password" name="password_confirmation" class="mt-1 block w-full border border-emerald-200 rounded-xl px-4 py-2 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" required>
                        </div>
