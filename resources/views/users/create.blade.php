                ...
                <form method="POST" action="{{ route('users.store') }}">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" class="mt-1 block w-full border border-[#E5E7EB] rounded-xl px-4 py-2 focus:ring-2 focus:ring-[#1F8F6A] focus:border-[#1F8F6A]" required>
                            @error('name')<p class="text-sm text-rose-600 mt-1">{{ $message }}</p>@enderror
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-700">Email</label>
                            <input type="email" name="email" value="{{ old('email') }}" class="mt-1 block w-full border border-[#E5E7EB] rounded-xl px-4 py-2 focus:ring-2 focus:ring-[#1F8F6A] focus:border-[#1F8F6A]" required>
